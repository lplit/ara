...
public class PositionProtocolImpl implements PositionProtocol, EDProtocol {
	...
	private void move(Node host){
		...
		if(!isMoving()){
			moving=true;
			// Random speed
			this.current_speed= (int) (CommonState.r.nextDouble()*
		...
		if(distance_to_next - distance < 0.0){
			double next_x =  ( distance_to_next * ( (current_destination.getX()
					- current_position.getX()) / distance )) +current_position.getX();
			double next_y =  ( distance_to_next * ( (current_destination.getY()
					- current_position.getY()) / distance )) +current_position.getY();
